...
function ComponentRenderer({ comp }) {
  ...
  switch (type) {
    ...
    case 'text':
      return (
        <div style={{
          ...
          color: props.textColor,
          fontSize: props.fontSize,
          fontWeight: props.fontWeight === 'bold' ? 700 : props.fontWeight === 'semibold' ? 600 : 400,
    ...
    case 'input':
      return (
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3 }}>
          {props.label && (
            <div style={{ fontSize: 11, color: '#6B7280', fontFamily: 'Nunito, sans-serif', fontWeight: 600 }}>
              {props.label}
            </div>
          )}
          <div style={{
    ...
    case 'checkbox':
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, height: '100%' }}>
          <div style={{
            width: 20, height: 20,
            border: `2px solid ${props.accentColor || '#6C63FF'}`,
            borderRadius: 5,
            backgroundColor: props.checked ? (props.accentColor || '#6C63FF') : 'transparent',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexShrink: 0,
          }}>
            {props.checked && <LucideIcons.Check size={13} color="white" strokeWidth={3} />}
          </div>
          <span style={{ color: props.textColor, fontSize: props.fontSize || 14, fontFamily: 'Nunito, sans-serif' }}>
            {props.label}
          </span>
        </div>
      );
      ...
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, height: '100%' }}>
          <div style={{
            ...
            {props.checked && (
              <div style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: props.accentColor || '#6C63FF' }} />
            )}
          </div>
          <span style={{ color: props.textColor, fontSize: props.fontSize || 14, fontFamily: 'Nunito, sans-serif' }}>
            {props.label}
    ...
    case 'image':
      return (
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: Math.min(position.width, position.height) * 0.25,
          color: '#9CA3AF',
          border: '2px dashed #D1D5DB',
        }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: Math.min(position.width, position.height) * 0.3 }}>🖼️</div>
            <div style={{ fontSize: 11, marginTop: 4, fontFamily: 'Nunito, sans-serif', color: '#9CA3AF' }}>Image</div>
          </div>
        </div>
      );
    ...
    case 'icon':
      return (
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <LucideIcon name={props.iconName} color={props.color} size={Math.max(16, iconSize)} />
        </div>
      );
    ...
    case 'header':
      return (
        <div style={{
          ...
          <span style={{
            color: props.textColor, fontSize: 18, fontWeight: 700,
    ...
    case 'switch':
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, height: '100%' }}>
          <span style={{ color: '#1F2937', fontSize: props.fontSize || 14, fontFamily: 'Nunito, sans-serif', flex: 1 }}>
            {props.label}
          </span>
          <div style={{
            width: 46, height: 26,
            backgroundColor: props.checked ? props.activeColor : '#D1D5DB',
            borderRadius: 13, position: 'relative', flexShrink: 0,
          }}>
            <div style={{
              position: 'absolute',
              top: 3, left: props.checked ? 23 : 3,
              width: 20, height: 20,
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, height: 6, backgroundColor: '#E5E7EB', borderRadius: 3 }}>
            <div style={{
              ...
              backgroundColor: props.activeColor, borderRadius: 3,
            }} />
            <div style={{
              position: 'absolute',
              left: `${props.value}%`, top: '50%',
              transform: 'translate(-50%, -50%)',
              width: 18, height: 18,
              backgroundColor: props.activeColor, borderRadius: '50%',
    ...
    case 'listitem':
      return (
        <div style={{
          ...
          <span style={{ flex: 1, color: props.textColor, fontSize: 14, fontFamily: 'Nunito, sans-serif', fontWeight: 600 }}>
            {props.label}
          </span>
    ...
    case 'avatar':
      return (
        <div style={{
          ...
          borderRadius: '50%',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: Math.min(position.width, position.height) * 0.45,
          overflow: 'hidden',
        }}>
          <LucideIcons.User size={Math.min(position.width, position.height) * 0.55} color="white" />
        </div>
      );
    ...
    case 'badge':
      return (
        <div style={{
          ...
          color: props.textColor,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: Math.min(position.width, position.height) * 0.38,
          fontWeight: 700, fontFamily: 'Nunito, sans-serif',
        }}>
          {props.count}
        </div>
    ...
    case 'separator':
      return (
        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center' }}>
          <div style={{ width: '100%', height: 1.5, backgroundColor: props.color }} />
        </div>
      );
    ...
    default:
      return (
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 12, color: '#6B7280', fontFamily: 'Nunito, sans-serif',
        }}>
          {type}
        </div>
      );
...
export default function Canvas({ canvasRef, interactionRef }) {
  ...
  const handleDrop = useCallback((e) => {
    ...
    dispatch({ type: 'ADD_COMPONENT', componentType, x, y });
  }, [dispatch, ref]);
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e) => {
      ...
      if (ds.type === 'move') {
        ...
        const newX = Math.max(0, Math.min(ds.origX + dx, CANVAS_W - ds.width));
        const newY = Math.max(0, Math.min(ds.origY + dy, CANVAS_H - ds.height));
        dispatch({ type: 'MOVE_COMPONENT', id: ds.compId, x: newX, y: newY });
    ...
    const onMouseUp = (e) => {
      ...
      if (ds.type === 'move') {
        ...
        if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
          const newX = Math.max(0, Math.min(ds.origX + dx, CANVAS_W - ds.width));
          const newY = Math.max(0, Math.min(ds.origY + dy, CANVAS_H - ds.height));
          dispatch({ type: 'COMMIT_MOVE', id: ds.compId, x: newX, y: newY });
        }
      } else if (ds.type === 'resize') {
        const screen = state.screens.find(s => s.id === state.activeScreenId);
  ...
  const sortedComponents = [...(screen.components || [])].sort(
        ...
        return (
          <div
            ...
            style={{
              ...
              outline: isSelected ? '2px solid #6C63FF' : undefined,
              ...
              <>
                {['nw', 'ne', 'sw', 'se'].map((h) => (
                  <div
                    ...
                    onMouseDown={(e) => handleResizeMouseDown(e, comp.id, h)}
                  />
                ))}
              </>
            )}
